import {Application, Graphics, ITextureDictionary, Loader, Sprite, Text, TextStyle} from "pixi.js";
import {
    BOARD_HEIGHT,
    BOARD_HEIGHT_PX,
    BOARD_WIDTH,
...
const TEXT_STYLE = new TextStyle({fontFamily: 'armada', fontSize: 18, fill: COLOR_WHITE});
...
export class Renderer {
    ...
    private static get gridXY(): [number, number] {
        return [CANVAS_WIDTH / 2 - BOARD_WIDTH_PX / 2, CANVAS_HEIGHT / 2 - BOARD_HEIGHT_PX / 2];
    }
    ...
    private renderActivePiece(game: GameState): void {
        this.activePieceSprites.forEach((sprite) => this.pixi.stage.removeChild(sprite));
        this.activePieceSprites = [];
    ...
    private renderGroundHint(game: GameState): void {
        this.groundHintSquares.forEach((sprite) => this.pixi.stage.removeChild(sprite));
        this.groundHintSquares = [];
    ...
    private renderNextPieces(game: GameState): void {
        this.nextPieceSprites.forEach((sprite) => this.pixi.stage.removeChild(sprite));
        this.nextPieceSprites = [];
